...
def get_random_code():
    # 随机字符
    codes = [[chr(i) for i in range(48, 58)], [chr(i)
...
def generate_captcha(width=140, height=60, length=4):
    ...
    for i in range(length):
        ...
        draw.text((width * 0.2 * (i + 1) + rand_len, height * 0.2 +
                  rand_len), c, font=font, fill=get_random_color())
    # 加入干扰线
    for i in range(3):
        x1 = randint(0, width)
        y1 = randint(0, height)
    ...
    # 加入干扰点
    for i in range(16):
        draw.point((randint(0, width), randint(0, height)),
                   fill=get_random_color())
        # 保存图片

    # img.save('aa.png', 'png')
    # text为生成的验证码，img为pillow格式的文本
    return text, pil_base64(img)
